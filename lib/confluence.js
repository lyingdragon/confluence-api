...
/**
 * Get pages under the space.
 *
 * @param {string} space
 * @param {Function} callback
 */
Confluence.prototype.getSpacePages = function(space, callback){

    request
        .get(this.config.baseUrl + this.config.apiPath + "/space/" + space + "/content/page")
        .auth(this.config.username, this.config.password)
        .end(function(err, res){
            processCallback(callback, err, res);
        });
